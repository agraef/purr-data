...
{
    if (ugen_loud) post("dereference %lx: %d", s, s->s_refcount);
    if (s->s_refcount <= 0)
...
void signal_setborrowed(t_signal *sig, t_signal *sig2)
{
    ...
    sig2->s_refcount++;
    if (ugen_loud) post("set borrowed %lx: from %lx vec %lx",
        sig, sig2, sig->s_vec);
}
...
{
    t_signal *s = signal_new((borrowed? 0 : DC_LENGTH(ugen_currentcontext)), nchans,
        DC_SR(ugen_currentcontext), 0);
    s->s_overlap = DC_OVERLAP(ugen_currentcontext);
    return s;
}
...
void ugen_stop(void)
{
#if 0   /* test to make sure we aren't leaving signal garbage */
    {
        ...
        for (sig = pd_this->pd_signals; sig; sig = sig->s_nextused)
        {
            if (sig->s_refcount)
...
void ugen_start(void)
{
    ugen_stop();
    ugen_sortno++;
    /* ugen_loud = 1; -- enable this for volumes of debugging output */
    ...
    for (u = dc->dc_ugenlist; u; u = u->u_next)
    {
        ...
        if (pd_class(zz) == voutlet_class)
        {
            ...
            voutlet_dspepilog((struct _voutlet *)zz,
                iosigs, calcsize, dsp_phase, period, frequency,
                    downsample, upsample, reblock, switched);
        }
    }
