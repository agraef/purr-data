...
Pd (Pure Data) is a graphical data-flow programming environment which is geared towards real-time interactive computer music and multimedia applications. Purr Data serves the same purpose, but offers some major GUI improvements and includes many 3rd party plug-ins. Like Pd, it runs on Linux, macOS and Windows, and is open-source throughout.
...
We also maintain a collection of Linux packages built from latest git sources for Arch Linux (via the [Arch User Repositories](https://aur.archlinux.org/) a.k.a. AUR) and recent Ubuntu releases (via [Launchpad](https://launchpad.net/~dr-graef)). More information and installation instructions can be found at [here](https://l2orkaur.bitbucket.io/) (Arch) and [here](https://l2orkubuntu.bitbucket.io/) (Ubuntu).
