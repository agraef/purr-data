...
static void packel_outelement(t_packel*x, int id, t_symbol*s,int argc,
                              t_atom*argv)
{
...
static void packel_anything(t_packel *x, t_symbol *s, int argc,
                            t_atom *argv)
{
  ...
  if(x->x_warningflag) {
    pd_error(x,
             "deprecation warning: you should only use lists for list data");
    x->x_warningflag=0;
  }
...
static void packel_free(t_packel *x)
{
  ...
  for(i=0; i<x->count; i++) {
    if(x->x_inlet &&x->x_inlet [i]) {
      inlet_free (x->x_inlet [i]);
    }
    if(x->x_outlet&&x->x_outlet[i]) {
      outlet_free(x->x_outlet[i]);
    }
  }

  if(x->position) {
    freebytes(x->position, x->count*sizeof(t_float));
  }
  if(x->x_inlet) {
    freebytes(x->x_inlet, x->count*sizeof(t_inlet*));
  }
  if(x->x_outlet) {
    freebytes(x->x_outlet, x->count*sizeof(t_outlet*));
  }
