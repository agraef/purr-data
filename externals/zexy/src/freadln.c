...
static void freadln_open (t_freadln *x, t_symbol *s, t_symbol*type)
{
  ...
  */
  ...
  if (!(x->x_textbuf = (char *) getbytes (MIN_FREADLN_LENGTH * sizeof(
      char)))) {
    pd_error(x, "out of memory!");
    freadln_close(x);
    return;
...
static void freadln_readline (t_freadln *x)
{
  ...
  do {
    ...
    if (!enlarge_cstr_if_required((const char**) &x->x_textbuf,
                                  &x->x_textbuf_length, min_length)) {
      pd_error(x, "out of memory");
      x->x_textbuf_length=0;
      freadln_close(x);
      freadln_done(x);
    ...
    if (!(items_read=fread(x->x_textbuf,sizeof(char),x->x_textbuf_length,
                           x->x_file))) {
      freadln_close(x);
      freadln_done(x);
      return;
  ...
  } while (((linebreak_pos=cstr_char_pos(x->x_textbuf,
                                         x->linebreak_chr[0]))==-1) &&
  ...
  if (abuf_length>0) {
    if (abuf->a_type==A_SYMBOL) {
      outlet_anything(x->x_message_outlet, atom_getsymbol(abuf), abuf_length-1,
                      abuf+1);
    } else {
