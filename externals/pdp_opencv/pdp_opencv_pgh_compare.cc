...
} t_pdp_opencv_pgh_compare;

static void pdp_opencv_pgh_compare_process_rgb(t_pdp_opencv_pgh_compare *x)
{
        ...
        (x->x_height != (t_int)header->info.image.height)) 
    {
    	...
    	post("pdp_opencv_pgh_compare :: resizing");
...
static void pdp_opencv_pgh_compare_sendpacket(t_pdp_opencv_pgh_compare *x)
{
    /* release the packet */
...
static void pdp_opencv_pgh_compare_process(t_pdp_opencv_pgh_compare *x)
{
   int encoding;
   t_pdp *header = 0;
   char *parname;
   ...
   if ( (header = pdp_packet_header(x->x_packet0))
	...
	/* pdp_opencv_pgh_compare_process inputs and write into active inlet */
	switch(pdp_packet_header(x->x_packet0)->info.image.encoding){

	case PDP_BITMAP_RGB:
            x->x_packet1 = pdp_packet_clone_rw(x->x_packet0);
            pdp_queue_add(x, (void*)pdp_opencv_pgh_compare_process_rgb, (void*)pdp_opencv_pgh_compare_sendpacket, &x->x_queue_id);
	    break;

	default:
	    /* don't know the type, so dont pdp_opencv_pgh_compare_process */
	    break;
...
static void pdp_opencv_pgh_compare_input_0(t_pdp_opencv_pgh_compare *x, t_symbol *s, t_floatarg f)
{
    ...
    if ((s == gensym("process")) && (-1 != x->x_packet0) && (!x->x_dropped))
    {
        ...
        pdp_opencv_pgh_compare_process(x);
    }
}

static void pdp_opencv_pgh_compare_input_1(t_pdp_opencv_pgh_compare *x, t_symbol *s, t_floatarg fpacket)
{
  t_pdp     *rightheader;
  short int *rightdata; 
  CvSeq     *contourr = NULL;
  ...
  CvRect    rect;
    ...
    if (s == gensym("register_rw") && ( x->x_bcontourr == NULL ) ) 
    {
       ...
       {
          post( "pdp_opencv_pgh_compare : error : null packet" );
          return;
       }
       ...
       {
          post( "pdp_opencv_pgh_compare : error : wrong right image size" );
          return;
       }
       ...
       try {
          ...
          post( "pdp_opencv_pgh_compare : error calculating contours" );
          return;
       }
    ...
    /* if this is a register_ro message or register_rw message, register with packet factory */
static void pdp_opencv_pgh_compare_free(t_pdp_opencv_pgh_compare *x)
{
  int i;
...
t_class *pdp_opencv_pgh_compare_class;


void *pdp_opencv_pgh_compare_new(t_floatarg f)
{
    int i;

    t_pdp_opencv_pgh_compare *x = (t_pdp_opencv_pgh_compare *)pd_new(pdp_opencv_pgh_compare_class);
...
static void pdp_opencv_pgh_compare_minsize(t_pdp_opencv_pgh_compare *x, t_floatarg f)
{
    if ( (int)f > 0 )
    {
       x->x_minsize = (int)f;
...
static void pdp_opencv_pgh_compare_criteria(t_pdp_opencv_pgh_compare *x, t_floatarg f)
{
    if ( f > 0.0 )
    {
       x->x_cdistance = f;
...
static void pdp_opencv_pgh_compare_clear(t_pdp_opencv_pgh_compare *x)
{
    x->x_bcontourr = NULL;
}
...
void pdp_opencv_pgh_compare_setup(void)
{
    ...
    post( "		pdp_opencv_pgh_compare");
    pdp_opencv_pgh_compare_class = class_new(gensym("pdp_opencv_pgh_compare"), (t_newmethod)pdp_opencv_pgh_compare_new,
    	(t_method)pdp_opencv_pgh_compare_free, sizeof(t_pdp_opencv_pgh_compare), 0, A_DEFFLOAT, A_NULL);

    class_addmethod(pdp_opencv_pgh_compare_class, (t_method)pdp_opencv_pgh_compare_input_0, gensym("pdp"), A_SYMBOL, A_DEFFLOAT, A_NULL);
    class_addmethod(pdp_opencv_pgh_compare_class, (t_method)pdp_opencv_pgh_compare_input_1, gensym("pdp1"), A_SYMBOL, A_DEFFLOAT, A_NULL);
    class_addmethod(pdp_opencv_pgh_compare_class, (t_method)pdp_opencv_pgh_compare_minsize, gensym("minsize"), A_DEFFLOAT, A_NULL);
    class_addmethod(pdp_opencv_pgh_compare_class, (t_method)pdp_opencv_pgh_compare_clear, gensym("clear"), A_NULL);
    class_addmethod(pdp_opencv_pgh_compare_class, (t_method)pdp_opencv_pgh_compare_criteria, gensym("criteria"), A_DEFFLOAT, A_NULL);
